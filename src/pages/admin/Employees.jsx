...
export default function Employees() {
  ...
  const getSalaryTypeLabel = (type) => {
    if (type === "daily") return "Kunlik";
    if (type === "weekly") return "Haftalik";
              ...
              {filteredEmployees.map((e) => (
                ...
                >
                  ...
                  <td className="p-4 text-emerald-400 whitespace-nowrap">
                    {Number(e.salaryAmount || 0).toLocaleString()} so‘m 

                    {e.salaryType === "daily" && " (k) "}
                    {e.salaryType === "weekly" && " (h) "}
                    {e.salaryType === "monthly" && " (o) "}
                  ...
                  <td className="p-4">
                    <span className="inline-flex items-center gap-2 bg-slate-800 px-3 py-1 rounded-xl text-sm">
                      <FiClock />
                      {Number(e.startTime ?? 0)}:00 → {Number(e.endTime ?? 0)}:00
